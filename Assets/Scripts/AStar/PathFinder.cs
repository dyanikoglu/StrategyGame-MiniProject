            ...
            lock (this)
            {
                ...
                while (_open.Count > 0)
                {
                    ...
                    for (var i = 0; i < (_diagonals ? 8 : 4); i++)
                    {
                        ...
                        // Addition to base a* pathfinding algorithm:
                        // We need to connect pathfinding algorithm to Unity Engine on here.
                        // Alternatively, we can store a wall matrix in app.model and check obstacles from here, but updating this wall list with each new frame won't be an efficient solution.
                        // Instead of that, we can do a raycast to each search node, and check if it's empty or not.
                        // On average runtime, this solution is more efficient than using a wall matrix.
                        var hit = Physics2D.Raycast(new Vector2(newLocationX + 0.5f, newLocationY + 0.5f), -Vector2.up);
                        if (hit.collider != null && !hit.collider.GetComponent<MapView>())
                        {
                            // This tile is not empty, pass it.
                            continue;
                        }
